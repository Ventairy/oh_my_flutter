...
void main() {
  testWidgets(
    'when details mount, it should show their external header',
    (tester) async {
      ...
      final header = tester.widget<FadeTransition>(
        find.descendant(
          of: find.ancestor(
            of: find.text('Details header'),
            matching: find.byType(MorphSibling),
          ),
  ...
  testWidgets(
    'when details are removed, it should restore the card header',
    (tester) async {
      final observer = MorphNavigatorObserver();
      await tester.pumpWidget(
        MaterialApp(
