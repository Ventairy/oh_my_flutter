...
void main() {
  group('InteractiveSwipeDismissBenchmarkRecordBuffer', () {
    test('when a record is added, it should not emit before flushing', () {
      ...
      InteractiveSwipeDismissBenchmarkRecordBuffer(emitted.add).add(<String, Object>{'path': 'environment'});
    ...
    test('when records are flushed, it should preserve their order', () {
      ...
      expect(emitted, <String>['$marker{"path":"first"}', '$marker{"path":"second"}']);
    });
    ...
    test('when a record exceeds Android log limits, '
      ...
      final emitted = <String>[];
      InteractiveSwipeDismissBenchmarkRecordBuffer(emitted.add)
        ..add(<String, Object>{'path': 'steady.trial_1', 'details': List<String>.filled(6000, 'x').join()})
        ..flush();
      ...
      expect(
        ...
        everyElement(
          allOf(
            ...
            startsWith(InteractiveSwipeDismissBenchmarkRecordBuffer.chunkMarker),
          ),
        ),
      );
    });

    test('when a record is chunked, it should preserve its exact JSON payload', () {
      final record = <String, Object>{'path': 'steady.trial_1', 'details': List<String>.filled(6000, 'x').join()};
      final emitted = <String>[];
      InteractiveSwipeDismissBenchmarkRecordBuffer(emitted.add)
        ..add(record)
        ..flush();
      final chunks = emitted
          .map((line) {
            final payload = line.substring(InteractiveSwipeDismissBenchmarkRecordBuffer.chunkMarker.length);
      ...
      expect(jsonDecode(utf8.decode(base64Decode(encodedPayload))), record);
    });

    test('when separate oversized records flush, '
