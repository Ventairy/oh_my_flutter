...
void main() {
  ui.FrameTiming frame({required int build, required int raster, required int total, required int vsync}) {
  ...
  test('when separate frame phases miss a 120 Hz budget, '
  ...
  test('when all work fits the refresh budget, it should pass its p99 gate', () {
    final report = NativeSelectableTextBenchmarkFrameReport.fromFrames(
  ...
  test('when no frame timings are supplied, it should reject the window', () {
    expect(
