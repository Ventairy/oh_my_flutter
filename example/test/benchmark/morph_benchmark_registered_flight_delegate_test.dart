...
void main() {
  testWidgets('when registered dense content transitions, '
      'it should display all twenty-four captured descendants', (tester) async {
  ...
  test('when surface values interpolate across the content switch, '
    ...
    final properties = delegate.lerpProperties(
      (decoration: const BoxDecoration(color: Colors.black), padding: const EdgeInsets.all(8), child: sourceChild),
      (
    ...
    expect(
      ...
      (true, const EdgeInsets.all(14), Color.lerp(Colors.black, Colors.white, 0.75)),
    );
  });
}
