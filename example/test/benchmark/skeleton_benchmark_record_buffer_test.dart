...
void main() {
  group('SkeletonBenchmarkRecordBuffer', () {
    test('when a record is added, it should not emit before flushing', () {
      final emitted = <String>[];
      SkeletonBenchmarkRecordBuffer(emitted.add).add(<String, Object>{'path': 'environment'});
    ...
    test('when records are flushed, it should preserve their order', () {
      ...
      expect(emitted, <String>['SKELETON_BENCHMARK {"path":"first"}', 'SKELETON_BENCHMARK {"path":"second"}']);
    });

    test('when a flushed buffer is flushed again, '
      ...
      expect(emitted, <String>['SKELETON_BENCHMARK {"path":"environment"}']);
    });

    test('when a record exceeds Android log limits, '
    ...
    test('when a record is chunked, '
      ...
      final record = <String, Object>{'path': 'acceptance', 'details': List<String>.filled(6000, 'x').join()};
      final emitted = <String>[];
      SkeletonBenchmarkRecordBuffer(emitted.add)
        ..add(record)
        ..flush();
      ...
      final chunks = emitted
          .map((line) {
            final Object? decoded = jsonDecode(line.substring(marker.length));
      ...
      final chunkKeys = chunks
          ...
          .toList(growable: false);
      final recordIdentifiers = <Object?>[for (final chunk in chunks) chunk['record']];
      final indexes = <Object?>[for (final chunk in chunks) chunk['index']];
      final counts = <Object?>[for (final chunk in chunks) chunk['count']];
      ...
      expect(
        <String, Object?>{
          ...
          'indexes': indexes,
          'counts': counts,
          'decoded': jsonDecode(utf8.decode(base64Decode(encodedPayload))),
        },
        <String, Object?>{
          'keys': List<List<String>>.generate(count, (_) => <String>['record', 'index', 'count', 'payload']),
    ...
    test('when separate oversized records are flushed, '
      ...
      for (final path in const <String>['first', 'second']) {
        buffer
          ..add(<String, Object>{'path': path, 'details': List<String>.filled(6000, path).join()})
          ..flush();
      }
      const marker = 'SKELETON_BENCHMARK_CHUNK ';
