...
void main() {
  group('MorphBenchmarkScenario', () {
    test('when all scenarios are selected, '
        'it should preserve the declared order', () {
      final ids = MorphBenchmarkScenario.selectedFrom('all').map((scenario) => scenario.id);
      expect(ids, <String>[
        'text',
        'column',
        'surface',
    ...
    for (final scenario in MorphBenchmarkScenario.values) {
      test('when ${scenario.id} is selected, '
          'it should resolve that scenario', () {
        expect(MorphBenchmarkScenario.selectedFrom(scenario.id), <MorphBenchmarkScenario>[scenario]);
      });
    }

    test('when an unknown scenario is selected, '
        'it should reject the identifier', () {
      expect(() => MorphBenchmarkScenario.selectedFrom('unknown'), throwsArgumentError);
    });

    test('when all scenarios are soaked, it should select the surface', () {
      expect(MorphBenchmarkScenario.soakTargetFor('all'), MorphBenchmarkScenario.surface);
    });

    test('when one scenario is soaked, it should preserve that scenario', () {
      expect(MorphBenchmarkScenario.soakTargetFor('column_unmatched'), MorphBenchmarkScenario.columnUnmatched);
    });

    test('when the same endpoint state rebuilds, '
        'it should preserve its child identity', () {
      final first = MorphBenchmarkScenario.text.endpointIdentity(child: 'title', destination: false);
      final second = MorphBenchmarkScenario.text.endpointIdentity(child: 'title', destination: false);
      expect(identical(first, second) || first == second, isTrue);
    });

    test('when the endpoint state changes, '
        'it should change its child identity', () {
      final source = MorphBenchmarkScenario.text.endpointIdentity(child: 'title', destination: false);
      final destination = MorphBenchmarkScenario.text.endpointIdentity(child: 'title', destination: true);
      expect(source == destination, isFalse);
    });

    test('when the dynamic watched snapshot scenario is inspected, '
    ...
    test('when the geometry-only watched snapshot scenario is inspected, '
    ...
    test('when the full-surface watched snapshot scenario is inspected, '
    ...
    test('when the nested fallback watched snapshot scenario is inspected, '
    ...
    test('when the static watched snapshot scenario is inspected, '
        'it should require no post-start mutation batches', () {
      const scenario = MorphBenchmarkScenario.watchSnapshotDense;
      expect((scenario.gatesWatchedSnapshotRefresh, scenario.snapshotMutationBatches), (true, 0));
    });
  });
}
